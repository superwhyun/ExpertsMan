...
// .wrangler/tmp/bundle-JzG3sq/strip-cf-connecting-ip-header.js
...
// src/utils/token.ts
async function generateToken(payload, secret, expiresInHours = 24) {
  const data = {
    ...payload,
    exp: Date.now() + expiresInHours * 60 * 60 * 1e3
  };
...
async function verifyToken(token, secret) {
  try {
    ...
    const expectedSig = Array.from(new Uint8Array(expectedBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
...
async function requireGodGod(c, next) {
...
__name(requireGodGod, "requireGodGod");
async function validateWorkspace(c, next) {
  const slug = c.req.param("slug");
  const workspace = await c.env.DB.prepare("SELECT * FROM workspaces WHERE slug = ?").bind(slug).first();
  if (!workspace) {
...
__name(validateWorkspace, "validateWorkspace");
async function requireWorkspaceAuth(c, next) {
...
__name(requireWorkspaceAuth, "requireWorkspaceAuth");

// src/routes/godgod.ts
var godgod = new Hono2();
godgod.post("/auth", async (c) => {
  const { password } = await c.req.json();
  if (password === c.env.GODGOD_PASSWORD) {
    const token = await generateToken({ type: "godgod" }, c.env.TOKEN_SECRET, 1);
...
godgod.get("/workspaces", requireGodGod, async (c) => {
  try {
    ...
    const result = [];
    for (const ws of workspaces2.results) {
      const count3 = await c.env.DB.prepare(
        "SELECT COUNT(*) as count FROM experts WHERE workspace_id = ?"
      ).bind(ws.id).first();
      result.push({
        ...ws,
        expertCount: count3?.count ?? 0
      });
    }
...
godgod.post("/workspaces", requireGodGod, async (c) => {
  try {
    ...
    ).bind(id, name, slug, password).run();
...
godgod.put("/workspaces/:id", requireGodGod, async (c) => {
  try {
    ...
    params.push(id);
    await c.env.DB.prepare(`UPDATE workspaces SET ${updates.join(", ")} WHERE id = ?`).bind(...params).run();
...
godgod.delete("/workspaces/:id", requireGodGod, async (c) => {
  try {
    const id = c.req.param("id");
    const experts = await c.env.DB.prepare("SELECT id FROM experts WHERE workspace_id = ?").bind(id).all();
    const statements = [];
...
godgod.post("/workspace-requests/:id/approve", requireGodGod, async (c) => {
  try {
    const id = c.req.param("id");
    const request = await c.env.DB.prepare("SELECT * FROM workspace_requests WHERE id = ?").bind(id).first();
    if (!request) {
...
workspaces.post("/:slug/auth", async (c) => {
  const workspace = c.get("workspace");
  const { password } = await c.req.json();
...
workspaces.get("/:slug/verify", requireWorkspaceAuth, async (c) => {
  const workspace = c.get("workspace");
  return c.json({
    valid: true,
    workspace: { id: workspace.id, name: workspace.name, slug: workspace.slug }
...
workspaces.get("/:slug/experts", requireWorkspaceAuth, async (c) => {
  try {
    const workspace = c.get("workspace");
    ...
    for (const expert of expertsResult.results) {
      ...
      const confirmedSlots = expert.confirmed_slots ? JSON.parse(expert.confirmed_slots) : [];
...
workspaces.get("/:slug/experts/:id", async (c) => {
  try {
    const workspace = c.get("workspace");
    const id = c.req.param("id");
...
workspaces.post("/:slug/experts", requireWorkspaceAuth, async (c) => {
  try {
    const workspace = c.get("workspace");
    const expert = await c.req.json();
    const existing = await c.env.DB.prepare("SELECT id FROM experts WHERE id = ?").bind(expert.id).first();
    if (existing) {
      await c.env.DB.prepare(
      ...
      await c.env.DB.prepare(
        ...
        expert.status || "none",
        expert.password || null,
        expert.createdAt || (/* @__PURE__ */ new Date()).toISOString()
      ).run();
    }
    return c.json({ success: true });
  } catch (error3) {
...
workspaces.delete("/:slug/experts/:id", requireWorkspaceAuth, async (c) => {
  try {
    const workspace = c.get("workspace");
    const id = c.req.param("id");
    await c.env.DB.batch([
...
workspaces.post(
  ...
  async (c) => {
    try {
      ...
      ).bind(slot.id, id, slot.date, slot.time, 0).run();
      return c.json({ success: true });
    } catch (error3) {
      console.error(error3);
      return c.json({ error: error3.message }, 500);
...
workspaces.post("/:slug/experts/:id/verify-password", async (c) => {
  try {
    ...
    if (existing) {
      ...
      ).bind(id, voterName, password).run();
...
workspaces.post("/:slug/experts/:id/vote", async (c) => {
  try {
    const id = c.req.param("id");
    const { voterName, selectedSlotIds } = await c.req.json();
    const expert = await c.env.DB.prepare("SELECT status FROM experts WHERE id = ?").bind(id).first();
...
workspaces.post(
  ...
  async (c) => {
    try {
      const id = c.req.param("id");
      await c.env.DB.prepare("UPDATE experts SET status = ? WHERE id = ?").bind("polling", id).run();
      return c.json({ success: true });
    } catch (error3) {
      console.error(error3);
      return c.json({ error: error3.message }, 500);
...
workspaces.post(
  ...
  async (c) => {
    try {
      const id = c.req.param("id");
      const { slotIds } = await c.req.json();
      ...
      ).bind("confirmed", JSON.stringify(slots.results), id).run();
...
workspaces.post("/:slug/experts/:id/select-slot", async (c) => {
  try {
    const workspace = c.get("workspace");
    const id = c.req.param("id");
    const { slot } = await c.req.json();
    ...
    ).bind("registered", JSON.stringify(slot), id, workspace.id).run();
    return c.json({ success: true });
  } catch (error3) {
    console.error("select-slot error:", error3);
    return c.json({ error: error3.message }, 500);
  }
});
workspaces.post("/:slug/experts/:id/no-available-schedule", async (c) => {
  try {
    const id = c.req.param("id");
    await c.env.DB.prepare("UPDATE experts SET status = ? WHERE id = ?").bind("unavailable", id).run();
    return c.json({ success: true });
  } catch (error3) {
    console.error(error3);
    return c.json({ error: error3.message }, 500);
...
// src/routes/workspace-requests.ts
var workspaceRequests = new Hono2();
...
var workspace_requests_default = workspaceRequests;
...
app.route("/api/godgod", godgod_default);
app.route("/api/workspaces", workspaces_default);
app.route("/api/workspace-requests", workspace_requests_default);
...
// .wrangler/tmp/bundle-JzG3sq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JzG3sq/middleware-loader.entry.ts
